...
/**
 * @brief Overkill for what we have but good practice
 * keeps track of mapped regions in case we'd need to unmap
 *
 * @param addr
 */
...
static int parse_elf64_secheaders(Elf64_Ehdr *e64hdr, uint8_t *fdata)
{
    ...
    // The ELF64 header tells us all we need
    elf_to_load.shdr_table = calloc(e64hdr->e_shnum * e64hdr->e_shentsize, sizeof(uint8_t));
...
static int map_loadable_segments(Elf64_Ehdr *e64_hdr, uint8_t *fdata, uint64_t *entry_point)
{
    ...
    for (int i = 0; i < e64_hdr->e_shnum; i++)
    {
        ...
        if (PT_LOAD == phdrs[i].p_type)
        {
            ...
            printf("[+] Loading segment @ 0x%08lx..0x%08lx with perms: ", phdrs[i].p_vaddr, phdrs[i].p_vaddr + phdrs[i].p_memsz);
            print_elf64_progheader_flags(phdrs[i].p_flags);
            printf("\n");

            // Initially make everything writable
            // We'll need to copy the segment into our mmap'd region
            res = mmap((void *)phdrs[i].p_vaddr, phdrs[i].p_memsz, PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
            if (MAP_FAILED == res)
            {
                perror("[!] mmap() failed!\n");
                printf("Errno: %d\n", errno);
...
static int map_loadable_segments(Elf64_Ehdr *e64_hdr, uint8_t *fdata, uint64_t *entry_point)
{
    ...
    for (int i = 0; i < e64_hdr->e_shnum; i++)
    {
        ...
        if (PT_LOAD == phdrs[i].p_type && 0 != phdrs[i].p_memsz)
        {
            ...
            // TODO
            // make this print cleaner...
            printf("[+] Loading segment @ 0x%08lx..0x%08lx with perms: ", aligned_addr, segment_len);
            print_elf64_progheader_flags(phdrs[i].p_flags);
            printf("\n");

            // Initially just make it easy and make everything writable
            // We'll need to copy the segment into our mmap'd region
            res = mmap((void *)aligned_addr, segment_len, PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
            if (MAP_FAILED == res)
            {
                perror("[!] mmap() failed!\n");
...
#else
#error "Please define either INSTRUCTOR_EX1 or INSTRUCTOR_EX2 based on the lab you are building for"
#endif
